...
from utils import clear, collect_dates, draw_table, format_date_display, header, confirm_prompt
...
def repeat_days():
    ...
    for i, day in enumerate(recent, 1):
        rows.append([
            ...
            f"${float(day['total_fare']):.2f}",
        ])
    draw_table(["", "Date", "Trips", "Total Fare"], rows)

    print()
    choice = input("\tPick a day [1-{}]: ".format(len(recent))).strip()
    ...
    draw_table(["Mode", "From", "To", "Total Fare"], rows)

    total_per_day = sum(float(t["fare"]) for t in trips)
